...
- Pretendard font asset은 `packages/design-system/src/assets`에 단일 원본으로 두고, 디자인시스템의 `fonts.css`에서 import해 `@font-face`를 정의합니다.
...
- 전역 폰트는 `@dongchimi/design-system/styles/fonts.css`를 runtime entry에서 명시적으로 import합니다.
