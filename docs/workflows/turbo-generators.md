...
- `gen:next-page`는 Next App Router route entry를 생성하므로 `client`만 허용합니다.
- `gen:react-page`, `gen:app-component`, `gen:domain-component`, `gen:domain-section`은 `client`와 `market-owner`를 허용합니다.
- `market-owner`는 Vite React 앱이므로 Next route generator의 대상이 아닙니다. React Router route object 연결은 제품 IA가 확정된 뒤 수동으로 추가합니다.
- `gen:domain-query`, `gen:domain-mutation`은 현재 `client`만 허용합니다. `market-owner`는 API/query baseline이 준비된 뒤 generator 대상에 추가합니다.
...
- `components`, `sections`, `hooks`, `fixtures`, `utils`는 page 폴더 아래에 둡니다.
...
```text
apps/client/src/domains/{domain}/api/{query-name}-api.ts
apps/client/src/domains/{domain}/hooks/use-{query-name}-query.ts
apps/client/src/domains/{domain}/hooks/use-{query-name}-query.spec.md
apps/client/src/domains/{domain}/model/
apps/client/src/domains/{domain}/query-keys.ts
```

`gen:domain-mutation`은 아래 파일을 생성합니다.

```text
apps/client/src/domains/{domain}/api/{mutation-name}-api.ts
apps/client/src/domains/{domain}/hooks/use-{mutation-name}-mutation.ts
apps/client/src/domains/{domain}/hooks/use-{mutation-name}-mutation.spec.md
apps/client/src/domains/{domain}/model/
```
...
- `market-owner` query/mutation generator는 `ky`, TanStack Query, `src/shared/api`, `src/shared/query` 기준이 생긴 뒤 추가합니다.
...
- `market-owner` API/query generator는 앱의 API/query baseline이 생기기 전까지 추가하지 않습니다.
