...
- 확인한 route
- 확인한 viewport와 Figma frame 또는 Screen ID
...
- 새 app-shared 또는 page-local component와 그 위치를 택한 이유
- Figma mismatch와 의도 또는 제약
...
page publishing PR은 구현 파일 목록보다 디자인 근거와 layout boundary를 먼저 설명합니다.

- Background: Figma frame, Screen ID, route, target viewport, 기존 layout 기준이 왜 부족했는지 적습니다.
- Tasks: design-system 탐색, app-shared/page-local 결정, route/layout 연결, 상태와 접근성 확인 흐름을 문단형으로 적습니다.
- Implementation Notes: `apps/client` mobile web layout 또는 `apps/market-owner` desktop operation layout 중 어느 기준을 적용했는지 적습니다.
- Evidence: browser route, viewport, screenshot 또는 preview URL, Figma mismatch, 사용한 design-system component를 적습니다.
- Risk / Follow-up: 실제 API, auth guard, 공통 layout 승격, DS component 승격처럼 후속 이슈로 남긴 범위를 적습니다.
