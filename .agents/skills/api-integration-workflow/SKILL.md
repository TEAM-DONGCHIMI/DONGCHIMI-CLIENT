...
- 조건부 읽기:
  ...
  - `docs/code-quality/frontend-fundamentals.md`: hook return shape, API boundary, cache/error 책임의 응집도/결합도 self-check 허브가 필요할 때
  ...
  - `docs/code-quality/frontend-logic-composition.md`: query/mutation hook, orchestration hook, payload mapping 책임을 나눌 때
...
7. Frontend Fundamentals 허브, 개별 기준 문서, 필요 시 logic composition 기준으로 API helper, query/mutation hook, usage site의 책임 분리와 예측 가능성을 self-check합니다.
...
- Frontend Fundamentals 기준 self-check를 수행했고, hook/API surface가 비자명하면 `frontend-fundamentals-review`를 추가로 적용했습니다.
