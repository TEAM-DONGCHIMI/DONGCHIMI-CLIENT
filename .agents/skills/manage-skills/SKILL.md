...
- UI 호출 메타데이터가 필요한 workflow skill이면 `.agents/skills/<name>/agents/openai.yaml`을 추가했는지
...
| ----------------------------- | ---------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `verify-agent-docs`           | agent harness와 markdown 문서 구조 검증                          | `AGENTS.md`, `README.md`, `docs/**`, `recipes/**`, `templates/**`, `.agents/**`, `.github/**`, `.agents/skills/**/agents/openai.yaml`                                                         |
| `verify-design-system-icons`  | 디자인시스템 아이콘 source/generated/export/SVG 정책 검증        | `packages/design-system/src/icons/**`, `packages/design-system/scripts/*icon*.mjs`, `packages/design-system/package.json`, root `package.json`, `.github/workflows/ci.yml`, `docs/architecture/design-system-icons.md`, `docs/workflows/ci.md`, `docs/workflows/local-development.md`, `recipes/add-icon.md` |
...
반복된 세션 학습은 아래 순서로 분류합니다.

```text
IF 기존 workflow skill의 입력/순서/예외가 부족함:
  -> workflow skill UPDATE
ELSE IF 검증에서 놓친 파일/명령/정책이 있음:
  -> verify skill UPDATE
ELSE IF 사람이 반복해서 따라야 할 절차가 빠짐:
  -> docs 또는 recipe UPDATE
ELSE IF 세션별 선호나 브라우저 맥락만 해당함:
  -> memory 또는 최종 요약에만 기록
ELSE IF 세 가지 이상 작업 유형에 반복 적용되고 기존 skill로 표현하기 어려움:
  -> 새 skill CREATE 검토
```
...
- `AGENTS.md` 변경은 `verify-agent-docs` 커버리지 대상입니다. 다만 단독 `AGENTS.md` 변경만으로 새 verify skill을 만들지는 않습니다.
