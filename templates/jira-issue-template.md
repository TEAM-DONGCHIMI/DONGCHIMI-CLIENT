...
| `apps/design-system-web` 디자인시스템 배포 웹 사이트 | `DCMDS-*`        |
| `packages/design-system` 디자인 시스템               | `DCMDS-*`        |

여러 앱, `packages/*`, 루트 설정, 문서, generator, CI처럼 제품 루트 변경은 `DCMFE-*`를 우선 사용합니다.
디자인시스템 package와 design-system-web은 같은 `DCMDS-*` project를 사용하고, 필요한 경우 Jira Component 또는 label로 `package`, `design-system-web`, `storybook`, `tokens`, `icons`를 구분합니다.
