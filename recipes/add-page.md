...
- 새 page 구조는 `apps/{app}/src/pages/{domain}/{page}/{PageName}.tsx`를 후보로 둡니다.
- Domain API helpers, query hooks, query keys는 `apps/{app}/src/pages/{domain}/{api|hooks|query-keys.ts}`에 둡니다.
- Page-local components와 sections는 `apps/{app}/src/pages/{domain}/{page}/{components|sections}`에 둡니다.
