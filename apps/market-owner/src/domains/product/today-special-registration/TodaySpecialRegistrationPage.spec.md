...
오늘의 특가 상품 등록 page가 protected sidebar layout 안에서 진입되는지 확인합니다. 실제 상품 등록 form과 API는 후속 flow 작업에서 구현합니다.
...
- success: `/products/today-special/new` route가 `오늘의 특가 상품 등록` heading을 렌더링합니다.
...
- 상품 form, validation, submit mutation은 후속 이슈 범위입니다.

## Accessibility

- heading order: page root는 visible `h1`으로 `오늘의 특가 상품 등록`을 제공합니다.
...
- [ ] `/products/today-special/new` route renders `오늘의 특가 상품 등록`
