...
- `hooks/use-today-special-product-registration.ts`
  ...
  - 이미지가 있으면 등록 payload의 `thumbnailUrl`에는 `objectKey`를 사용하고, 화면 snapshot에는 `VITE_PUBLIC_S3_BASE_URL`과 결합한 공개 URL을 사용합니다.
  - base URL이 없으면 화면 표시 URL을 만들 수 없으므로 등록을 시작하지 않습니다.
...
- `등록 완료`
  ...
  - 등록 성공 화면 snapshot에는 S3 base URL과 `objectKey`를 결합한 공개 URL을 사용합니다.
