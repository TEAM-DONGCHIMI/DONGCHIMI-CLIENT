...
오늘의 특가 상품 수정 page가 protected sidebar layout 안에서 진입되고, 수정하기 tab과 sidebar active state가 같은 route를 바라보는지 확인합니다.
...
- success: `/products/today-special/edit` route가 `오늘의 특가 상품 수정` heading을 렌더링합니다.
...
- 수정 table, filter, pagination, mutation은 후속 이슈 범위입니다.

## Accessibility

- heading order: page root는 visible `h1`으로 `오늘의 특가 상품 수정`을 제공합니다.
...
- [ ] `/products/today-special/edit` route renders `오늘의 특가 상품 수정`
