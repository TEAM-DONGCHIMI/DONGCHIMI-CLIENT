...
- query: auth store의 `marketId`로 `useProductListInfiniteQuery({ marketId, type: 'DAILY', sort })`
- pagination: 서버의 `nextCursor`를 다음 요청의 `cursor`로 전달하고 목록 하단 진입 시 다음 페이지를 이어 붙입니다.
- mutation: 개별 삭제, 일괄 삭제, `dealType=DAILY` 상품 초기화
- fixture: 사용하지 않음
...
- 초기화 확인 시 `dealType: DAILY`, `forceDelete: true`로 전체 삭제 API를 요청합니다.
