...
- panel: clicking `POS에서 엑셀 파일 받는 방법 보기` opens the right POS guide modal panel with a right-to-left slide-in animation, a `2.8rem × 4rem` close button frame aligned at the panel's `1.8rem` top and `1.945rem` left inset so its visual icon edge follows the `2.5rem` content grid, the two-line title (`POS에서 엑셀 파일을` / `이렇게 다운 받으시면 돼요.`), and three stacked guide image placeholders; Escape, backdrop click, or the close button hides it and restores focus.
