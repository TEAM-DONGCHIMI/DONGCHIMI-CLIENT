...
describe('PosExcelGuidePanel', () => {
  ...
  it('renders the POS guide image as a modal dialog when open', () => {
    ...
    const guideImage = within(panel).getByRole('img', {
      name: /POS 엑셀 상품 등록 안내: 1\. 상품관리 또는 상품목록 메뉴/,
    });

    expect(guideImage).toHaveAttribute('src', '/images/pos-excel-guide.webp');
    expect(guideImage).toHaveAttribute('width', '360');
    expect(guideImage).toHaveAttribute('height', '722');
  });
