...
- Header logo는 로그인과 동일한 `Img_pavicon.svg`를 92×32 크기로 표시합니다.
...
- `이전`을 누르는 즉시 route 이탈 상태로 전환해 예약된 자동 저장 callback이 추가 PUT 요청을 보내지 않도록 하고, unmount cleanup에서 interval을 제거합니다.
- `등록 완료` 클릭 시 변경 여부나 직전 자동 저장 payload와의 중복 여부와 관계없이 현재 전체 row payload를 임시 저장 API로 한 번 더 전송합니다.
- 마지막 임시 저장이 성공하면 `/leaflets/share`로 이동하고, 실패하면 결과 확인 페이지에 머물며 오류 toast를 표시합니다.
...
- [x] final registration always saves the full current payload and routes to `/leaflets/share` only after success
