...
- uploading: 이미지 선택 즉시 Presigned URL 발급과 S3 PUT을 시작하며, 업로드가 끝나기 전까지 `등록 완료`를 막습니다.
- uploadError: 최신 이미지 업로드가 실패하면 preview를 유지하고 오류 toast를 표시하며, 새 이미지를 선택해 재시도하기 전까지 `등록 완료`를 막습니다.
...
- Breadcrumb copy는 `행사 할인 상품 등록 / 상품 등록 결과 확인`입니다.
- Header logo는 app-shared `Img_pavicon.svg`를 92×32 크기로 표시합니다.
- Page heading은 Figma 기준 `상품 등록 결과 확인`입니다.
...
- 이미지 선택은 일반 field draft debounce와 분리합니다. 파일 선택 즉시 Presigned `uploadUrl` 발급과 S3 PUT을 시작하고, 업로드 완료 전에는 draft 저장 single-flight를 점유하지 않습니다.
- 변경 이미지 업로드가 완료되면 응답의 `objectKey`를 임시 저장 API의 `thumbnailUrl`에 담아 추가 debounce 없이 즉시 저장합니다. 이미 draft PUT/GET이 진행 중이면 중첩 요청 대신 최신 payload를 후속 저장으로 직렬화합니다.
- 같은 row에서 이미지를 연속 선택하면 마지막 선택의 업로드 결과만 유효합니다. 먼저 선택한 이미지의 늦은 성공/실패 응답은 preview, `objectKey`, 오류 상태를 덮어쓰지 않습니다.
- 화면 미리보기는 로컬 preview URL을 유지하며, draft PUT/GET이 성공해도 해당 요청에 포함된 `objectKey`와 현재 이미지가 같은 경우에만 preview와 업로드 상태를 정리합니다.
- 기존 분석 결과 이미지가 있는 row는 이미지 자체를 `이미지 변경` 버튼으로 노출해 다시 업로드할 수 있습니다.
- 이미지가 없는 row는 `이미지 추가` action을 표시하고, 이미지가 등록되면 즉시 썸네일을 표시합니다.
- 서버 `failCount`, 저장되지 않은 row revision, 로컬 validation error, 이미지 업로드 진행/실패, PUT/GET 진행 상태 중 하나라도 남아 있으면 하단 `등록 완료` 버튼을 disabled로 둡니다.
...
- [x] `/products/registration-result` route renders `상품 등록 결과 확인`
...
- [x] product image upload starts immediately without waiting for the field debounce
- [x] successful image upload triggers an immediate draft save with the S3 `objectKey`
- [x] product image upload does not occupy the draft PUT/GET single-flight lane
- [x] a late response from an older image selection cannot replace the latest image state
- [x] image upload pending/error states disable final registration and reselecting retries the upload
- [x] a text-only save cannot clear a newer pending image preview
