...
- success: 수정 완료 시 `정보가 변경되었습니다.` toast를 표시하고 현재 값을 pristine 기준으로 재설정한 뒤 홈으로 replace 이동합니다. 저장 성공에 따른 이동은 dirty 이탈 확인을 표시하지 않습니다.
...
- 성공하면 상세 query를 무효화하고 dirty 상태를 초기화한 뒤 완료 Toast를 표시하고 홈으로 replace 이동합니다.
