...
- Jira: DCMSM-27
- Related Jira: DCMSM-32 홈 상품 0건 딤드 상태 UI, DCMSM-38 헤더 상품 검색 공통화, DCMSM-73 상품 검색 API 연동
...
퍼블리싱합니다. 상품 검색은 product domain query를 사용하고, 대시보드/공유 데이터는 별도 API 작업 범위를 따릅니다.
...
- error: 알 수 없는 route는 router fallback에서 처리합니다. 상품 검색 API 또는 response validation이
  실패하면 검색 패널에 `상품 정보를 불러오지 못했어요.` error 상태를 표시합니다.
...
- query: `useProductSearchQuery({ marketId: 1, keyword, size: 10 })`
- mutation: none
- fixture:
