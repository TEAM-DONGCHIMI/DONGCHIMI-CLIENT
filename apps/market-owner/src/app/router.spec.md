...
- Jira: DCMSM-15
- Related Jira: DCMSM-17, DCMSM-20, DCMSM-24, DCMSM-26, DCMSM-33, DCMSM-72, DCMSM-103
- Screen ID: APPJAM login/signup, 행사 할인 상품 등록 flow, sidebar/edit tabs screenshots
- Route: `/`, `/login`, `/signup`, `/products/today-special/new`, `/products/event-discount/new`, `/products/today-special/edit`, `/products/event-discount/edit`, `/markets/information-registration`, `/markets/information-management`, `/products/registration-result`, `/leaflets/share`, `*`
- Owner: FE
- Status: Implemented
...
DCMSM-17부터 lazy route page는 앱 전용 `AsyncBoundary`를 통해 Suspense와 render error를 처리하며, route 전환 중에는 별도 로딩 패널을 표시하지 않습니다.
...
- Sidebar footer의 `마트 정보 관리` 링크는 `/markets/information-management`로 이동합니다.
...
- Not Found 화면의 `메인으로 가기` CTA는 홈 route로 이동하고 이후 인증 판단은 기존 `ProtectedRoute`에 위임합니다.
...
- Protected no-sidebar layout: `/markets/information-registration`, `/markets/information-management`, `/products/registration-result`
- Fallback: `*`

## States

- loading: lazy route page는 `AsyncBoundary`의 `null` Suspense fallback으로 처리해 별도 로딩 패널을 표시하지 않습니다.
...
- navigation: sidebar footer의 `마트 정보 관리`는 `/markets/information-management`로 이동합니다.
...
- [ ] browser route: `/markets/information-management`
- [ ] browser route: `/products/registration-result`
- [ ] browser route: `/leaflets/share`
- [ ] sidebar does not render `오늘의 전단 공유` item
- [ ] sidebar `마트 정보 관리` 링크가 마트 정보 관리 route로 이동
- [ ] lazy route 전환 중 공통 로딩 패널을 렌더링하지 않음
...
- 상품 등록 결과 확인 화면 본문 UI는 DCMSM-20에서 route-local page로 구현하고, 실제 API/polling/최종 등록 완료 이동은 후속 flow 이슈에서 확정합니다.
