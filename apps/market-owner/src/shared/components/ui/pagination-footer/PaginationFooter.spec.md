...
`PaginationFooter`는 사장님 앱의 table/list 하단에서 총 개수, 현재 표시 범위, 페이지당 표시 개수, 페이지 이동 UI를 함께 보여주는 app shared 컴포넌트입니다.
...
- [x] page size control은 추후 Dropdown 교체가 가능하도록 별도 prop과 callback으로 분리합니다.

## Props

- `totalCount`: 전체 item 개수입니다.
- `rangeStart` / `rangeEnd`: 현재 페이지에서 표시 중인 item 범위입니다.
- `pageSize`: 현재 페이지당 표시 개수입니다.
- `onPageSizeClick`: page size control을 눌렀을 때 호출됩니다. 없으면 static chip처럼 표시합니다.
...
- `pageSizeControlLabel`, `pageSizeSuffix`: page size control 문구를 조정합니다.

## States

- default: total count, current range, page size, page navigation을 표시합니다.
...
- static page size: `onPageSizeClick`이 없으면 page size control을 button이 아닌 static chip으로 렌더링합니다.
...
4. page size control을 누르면 `onPageSizeClick`을 호출합니다.
5. 실제 Dropdown open, API refetch, query string sync는 실행하지 않습니다.
...
- page size: 3.2rem high chip, neutral 20 1px border, 0.8rem radius, chevron-down icon placeholder
- navigation: design-system `Navigation`의 chevron buttons, `NumButton` page buttons, page gap을 그대로 사용
- responsive: 좁은 부모에서는 horizontal overflow를 허용해 text/control overlap을 피합니다.
...
- 이전/다음/page size control은 native button keyboard interaction과 focus-visible을 유지합니다.
