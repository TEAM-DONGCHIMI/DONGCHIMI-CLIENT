...
test('행사 할인 무한 목록을 뒤로가기에서 복원하고 상세 새로고침 후 token을 무효화한다', async ({
  page,
}) => {
  ...
  const allCategoryButton = page.getByRole('button', { exact: true, name: '전체' });

  await expect(allCategoryButton).toHaveAttribute('aria-pressed', 'true');
  await expect(page.getByRole('link', { name: '전체 행사상품 상품 보기' })).toBeVisible();
  await expect(targetProduct).not.toBeAttached();
  expect(
    await page.evaluate(() => {
      return Object.keys(window.sessionStorage).filter((key) => {
        return key.startsWith('market-products-scroll-restoration:');
      });
    }),
  ).toEqual([]);
});
