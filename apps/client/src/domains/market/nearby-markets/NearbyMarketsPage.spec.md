...
- market query ownership: `NearbyMarketsPage`가 `useGetNearbyMarketsInfiniteQuery({ keyword, lat, lng })`를 한 번만 호출하고, 조회 결과(마트 목록/에러/페이지네이션 액션)를 `NearbyMarketsMapSection`과 `NearbyMarketsMarketListSection`에 각각 필요한 만큼 props로 내려줍니다(지도와 목록이 동일한 검색 결과를 공유한다는 것을 명시적으로 드러내기 위해 각 섹션이 훅을 개별 호출하지 않습니다). 그 결과 목록에서 무한스크롤로 다음 페이지를 불러오면 지도 마커도 함께 늘어납니다 — 지도와 목록이 같은 검색 결과를 보여주는 의도된 동작입니다.
