...
- 아이콘 아래 10px 간격으로 `근처에 등록된 마트가 아직 없어요` 문구를 중앙 정렬로 표시합니다.
- 검색어가 있는 빈 결과는 `검색 결과가 없어요` 문구를 표시합니다.
- 등록된 마트가 응답에 포함되더라도 `productCount`가 0이면 마트 리스트에는 노출하지 않습니다.
- 등록된 마트가 응답에 포함되더라도 `productCount`가 0이면 지도 마커도 노출하지 않습니다.
- 위치 기반 응답 순서를 가까운 마트 우선 순서로 보고, 목록에서는 등록 상품이 있는 마트 중 영업중 마트를 먼저 노출합니다.
- 마트 카드 상품은 오늘의 특가와 행사 할인 상품이 합쳐진 `previewProducts` 응답 순서를 유지해 최대 3개만 노출합니다.
...
- empty search: 검색어가 있는 빈 결과는 `검색 결과가 없어요` 문구를 표시하고 빈 상태 이미지는 표시하지 않습니다.
...
- 위치 권한 허용 여부와 관계없이 input은 항상 읽기 전용입니다. 클릭하면 우편번호 검색 팝업을 엽니다.
- 위치 권한이 허용되면 현재 위치 좌표를 Kakao reverse geocoder로 변환한 주소 텍스트를 input에 그대로 표시합니다. 권한이 없거나 아직 변환되지 않은 경우에는 `현재 위치를 검색해주세요` placeholder를 표시합니다.
- 우편번호 검색으로 주소를 선택하면 그 검색어가 현재 위치 주소 텍스트보다 우선해 input에 표시됩니다.
- 우편번호 검색 결과가 선택되면 Kakao geocoder로 좌표를 변환하고 해당 좌표 기준으로 목록과 지도 마커를 다시 조회합니다.
- 위치 마커는 검색한 위치가 있으면 검색 위치만, 없으면 현재 위치만 하나로 표시하며 Kakao Maps 기본 마커 스타일을 사용합니다.
- 마트 목록은 `useGetNearbyMarketsInfiniteQuery` 결과를 flatten한 뒤 `productCount > 0`인 마트만 남기고, 영업중 마트를 우선 전달합니다.
- 같은 영업 상태의 마트는 위치 기반 API 응답 순서를 유지합니다.
- 각 마트 카드의 상품 목록은 `previewProducts` 앞 3개만 사용합니다. 등록 시간 기준 정렬과 같은 등록 기간 상품 랜덤 선택은 현재 응답 필드에 근거가 없으므로 API 응답 순서를 신뢰합니다.
