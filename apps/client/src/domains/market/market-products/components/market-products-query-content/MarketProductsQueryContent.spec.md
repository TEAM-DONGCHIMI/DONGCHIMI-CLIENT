...
`MarketProductsPage` 안에서 마트 상세 query를 실행하고, 마트 정보와 인기 상품 TOP3 영역의 server-state를 표현합니다.
...
MarketProductsQueryContent
  ...
  TodaySpecialProductsSection (fixture, out of current API scope)
...
- non-owned behavior: URL parsing, header/back navigation, 오늘의 특가와 행사 할인 query는 소유하지 않습니다.
