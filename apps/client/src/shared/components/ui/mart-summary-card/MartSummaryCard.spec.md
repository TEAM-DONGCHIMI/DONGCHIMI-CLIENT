...
- [x] 영업 상태는 `영업중` / `영업 종료` 텍스트로 표시합니다. 위치 정보는 이 카드에서 표시하지 않습니다.
- [x] 상품은 `MarketCard`를 조합하며 사용부가 넘긴 목록을 가로 스크롤로 노출합니다.
- [x] 영업 상태 텍스트와 할인 chip 사이는 `0.6rem`으로 분리합니다.
...
- default: 마트 프로필, 마트명, 영업 상태 텍스트, `Chip`으로 만든 할인 개수 chip, 액션 버튼, 상품 목록을 표시합니다.
...
- `Closed`: `isOpen`이 `false`일 때 `영업 종료` 텍스트가 표시되는 상태를 확인합니다.
