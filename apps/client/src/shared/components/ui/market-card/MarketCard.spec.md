...
- [x] 할인 chip은 디자인시스템 `PointChip`을 사용하고 `size="mobile"`로 4rem 크기를 적용합니다.
...
- `saleChipLabel?: string`: 할인 chip 안에 표시할 텍스트입니다. `10%` 같은 값은 호출부에서 주입합니다.
- `className`과 `article` native props는 레이아웃 조정 범위에서만 사용합니다.

## States

- default: 이미지, 상품명, 가격을 표시합니다.
- sale chip: `hasSaleChip`이 `true`이면 우상단에 `PointChip size="mobile"`을 표시합니다.
