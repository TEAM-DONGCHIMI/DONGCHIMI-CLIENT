...
- `trailingActionDisabled`: input은 편집 가능한 상태로 유지하면서 trailing action만 비활성화하는 선택 prop입니다.
...
- Field only: 48px
- Label + Field + Error: 98px (`22 + 6 + 48 + 4 + 18`)
- Field + Error: 70px (`48 + 4 + 18`)
...
- trailing action disabled: input은 활성 상태로 유지하고 action button에만 native `disabled`를 적용합니다.
...
- field-to-error gap: `0.4rem` (기본 root font-size 10px 기준 4px)
