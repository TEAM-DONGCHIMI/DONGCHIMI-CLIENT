...
- type: `ReactNode`
- required: `false`
- description: input 위에 표시하는 visible label입니다.
...
- 빈 문자열인 label, error message, error icon은 렌더링하지 않습니다. 숫자 `0`은 유효한 React content로 처리합니다.
...
- overflow: input에 `min-width: 0`; 긴 값은 native horizontal scroll behavior를 유지
